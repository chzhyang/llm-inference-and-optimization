"""
Convert huggingface llama model. Use https://huggingface.co/meta-llama
"""
...
def split_and_convert_process(
    ...
    if (
        ...
        qkvcols = val.shape[-1]
        head_size = int(qkvcols / (int(num_attention_heads) +
        ...
        for j in range(factor):
            val = np.concatenate(
                (q_split_vals[j], k_split_vals[j], v_split_vals[j]), axis=-1)
...
def split_and_convert(args):
    ...
    try:
        ...
        num_attention_heads = config["llama"]["head_num"] = str(
    ...
    for name, param in state_dict.items():
        ...
        if "self_attn.q_proj.weight" in name:
            ...
            model_named_parameters[name.replace(
                "self_attn.q_proj.weight", "attention.query_key_value.weight")] = qkv
        # for merged weights, skip
            ...
            model_named_parameters[name] = param.permute(
                1, 0) if len(param.shape) == 2 else param
...
if __name__ == "__main__":
    ...
    parser.add_argument("--processes", "-p", type=int,
                        help="processes to spawn for conversion (default: 8)", default=8)
    parser.add_argument("--weight_data_type", "-d", type=str,
