...
int main(int argc, char **argv) {
    ...
    args.add("verbose", '\0', "Print detailed info"); //default is false
    args.add("warmup", '\0', "Warm up"); //default is false
    ...
    std::cout << "[INFO] Load tokenizer" << std::endl;
    ...
    // Warmup
    if (warmup) {
        std::cout << "[INFO] Warmup" << std::endl;
        std::string warm_prompt = "Once upon a time, there existed a little girl who liked to have adventures.";
        std::vector<int> warm_input = tokenizer->encode(warm_prompt);
        model.config(/*maxLen*/ 100, /*numBeams*/ 1, /*numBeamHypsToKeep*/ 1, /*lenPenalty*/ 1.0,
                /*doEarlyStopping*/ false, /*eosTokenId*/ -1, /*padTokenId*/ -1,
                /*doSample*/ false, /*temperature*/ 1.0,
                /*topK*/ 50.0, /*topP*/ 1.0, /*repetitionPenalty*/ 1.0);
        model.input(warm_input, /*batchSize*/1);
        while (!model.isDone()) {
            model.generate();
        }
        model.finalize();
        // auto warm_result = model.finalize();
        // std::vector<std::string> warm_result_str = tokenizer->batchDecode(warm_result, 1);
        // std::cout << "[INFO] Warmup result: " << std::endl;
        // for (auto str : warm_result_str) {
        //         // std::cout << "==============================================" << std::endl;
    ...
    std::cout << "[INFO] Prompt length is: " << inputSize << std::endl;
    std::cout << "[INFO] Start to generate" << std::endl;
    for (int i = 0; i < loop; ++i) {
        ...
        if (isMaster && secondIdCount > 0) {
            ...
            std::cout << "[INFO] Next token time: " << avgDuration << " ms" << std::endl;
        }
        auto result = model.finalize();

        if (isMaster) {
            
            std::cout << "[INFO] Generation length is: " << secondIdCount+1 << std::endl;
            std::cout << "[INFO] Final output is: ";
