...
extern "C" {
    void generate(const char* model_path, const char* token_path, const char* dtype_name, char* prompt, int outputLen=100, int batchSize=1, bool doSample=false, int numBeams=1, bool streamingOutput=false, bool verbose=true, int* prompt_len, int* generation_len, float* first_token_time, float* next_token_time) {
        std::string modelPath(model_path);
        std::string modeltype = getModelType(modelPath);
        std::string tokenPath(token_path);
        auto *tokenizer = getTokenizer(modeltype, tokenPath);
        ...
        for (int i = 0; i < loop; ++i) {
            ...
            if (!model.isDone()) {
                ...
                Timer firstTimer;
                firstIds = model.generate();
                auto firstDuration = firstTimer.getTime();
                if (isMaster) { // Todo ???? which node generate the first token 
                    *first_token_time = firstDuration;
                    std::cout << "[INFO] First timer result: "<< firstDuration << std::endl;
                }
            }
            ...
            if (isMaster && secondIdCount > 0) {
                auto avgDuration = timerSecond.getTime() / float(secondIdCount);
                *prompt_len = inputSize;
                *generation_len = secondIdCount + 1;
                *next_token_time = avgDuration;
                if (verbose) {
                    std::cout << std::endl << "[INFO] Second token time: " << avgDuration << " ms" << std::endl;
                }
            }
